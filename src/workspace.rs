...
pub fn create(printer: &mut Printer, space_name: &String) -> anyhow::Result<()> {
...

    {
        ...
        for (spaces_key, dependency) in workspace.repositories.iter() {
            let (bare_repository, execute_later) = git::BareRepository::new(section.printer, spaces_key, &dependency.git)?;
            execute_batch.add(spaces_key, execute_later);

            section.printer.info(spaces_key, &bare_repository)?;

            let (worktree, execute_later) = bare_repository.add_worktree(section.printer, &directory)?;
            execute_batch.add(spaces_key, execute_later);
            execute_batch.add(spaces_key, worktree.switch_new_branch(section.printer, dependency)?);
...
impl<'a> State<'a> {
    ...
    fn sync_dependencies(&mut self) -> anyhow::Result<()> {
        ...
        loop {
            if let Some((bare_repository, dependency)) = self.all_deps.pop_back() {
                ...
                let (worktree, execute_later) = bare_repository.add_worktree(section.printer, self.full_path)?;
                ...
                if self.deps_map.contains_key(&bare_repository.spaces_key) {
                    ...
                    execute_batch.add(&bare_repository.spaces_key, worktree.checkout(section.printer, &dependency)?);
                    execute_batch.add(&bare_repository.spaces_key, worktree.checkout_detached_head(section.printer)?);
                }

                let spaces_deps = worktree.get_deps()?;
                if let Some(spaces_deps) = spaces_deps {
                    section.printer.info("deps", &spaces_deps)?;
                    for (key, dep) in spaces_deps.deps.iter() {
                        section.printer.info(key, dep)?;
                        let (bare_repository, execute_later) = git::BareRepository::new(section.printer, key, &dep.git)?;
    ...
    fn export_buck_config(&mut self) -> anyhow::Result<()> {
        ...
        let heading = printer::Heading::new(self.printer, "Configuration")?;
        if let Some(buck) = workspace.buck.as_mut() {
            ...
            heading.printer.info("buck", &"No buck config found")?;
        }

        heading.printer.debug("workspace", &workspace)?;
