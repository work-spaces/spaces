...
pub fn globals(builder: &mut GlobalsBuilder) {
    ...
    fn get_platform_name() -> anyhow::Result<String> {
        evaluation_profile::profile_builtin_call("info", "get_platform_name", || {
    ...
    fn get_supported_platforms() -> anyhow::Result<Vec<String>> {
        evaluation_profile::profile_builtin_call("info", "get_supported_platforms", || {
    ...
    fn is_ci(eval: &mut Evaluator) -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_ci", || {
    ...
    fn get_execution_phase(eval: &mut Evaluator) -> anyhow::Result<String> {
        evaluation_profile::profile_builtin_call("info", "get_execution_phase", || {
            let ctx = get_eval_context(eval)?;
    ...
    fn is_platform_windows() -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_platform_windows", || {
    ...
    fn is_platform_macos() -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_platform_macos", || {
    ...
    fn is_platform_linux() -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_platform_linux", || {
    ...
    fn is_platform_x86_64() -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_platform_x86_64", || {
    ...
    fn is_platform_aarch64() -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "is_platform_aarch64", || {
    ...
    fn abort(message: &str) -> anyhow::Result<NoneType> {
        evaluation_profile::profile_builtin_call("info", "abort", || {
    ...
    fn get_cpu_count() -> anyhow::Result<i64> {
        evaluation_profile::profile_builtin_call("info", "get_cpu_count", || {
    ...
    fn parse_log_file<'v>(
        ...
        evaluation_profile::profile_builtin_call("info", "parse_log_file", || {
            let heap = eval.heap();
    ...
    fn get_path_to_store(eval: &mut Evaluator) -> anyhow::Result<String> {
        evaluation_profile::profile_builtin_call("info", "get_path_to_store", || {
            let ctx = get_eval_context(eval)?;
    ...
    fn get_path_to_spaces_tools(eval: &mut Evaluator) -> anyhow::Result<String> {
        evaluation_profile::profile_builtin_call("info", "get_path_to_spaces_tools", || {
            let ctx = get_eval_context(eval)?;
            // Use cached value from EvalContext to avoid lock contention
    ...
    fn get_log_divider_string() -> anyhow::Result<String> {
        evaluation_profile::profile_builtin_call("info", "get_log_divider_string", || {
    ...
    fn set_minimum_version(version: &str, eval: &mut Evaluator) -> anyhow::Result<NoneType> {
        evaluation_profile::profile_builtin_call("info", "set_minimum_version", || {
    ...
    fn set_required_semver(required: &str) -> anyhow::Result<NoneType> {
        evaluation_profile::profile_builtin_call("info", "set_required_semver", || {
    ...
    fn check_required_semver(required: &str) -> anyhow::Result<bool> {
        evaluation_profile::profile_builtin_call("info", "check_required_semver", || {
    ...
    fn set_max_queue_count(_count: i64) -> anyhow::Result<NoneType> {
        evaluation_profile::profile_builtin_call("info", "set_max_queue_count", || Ok(NoneType))
    }
}
