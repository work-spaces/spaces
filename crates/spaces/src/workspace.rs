...
fn get_spaces_env_reproducible(is_reproducible: bool) -> environment::Any {
    environment::Any {
        ...
        value: environment::Value::Assign(environment::AssignValue {
            value: is_reproducible.to_string().into(),
        }),
        help: Some("`true` if workspace has no repos checked out on branches".into()),
...
impl Workspace {
    ...
    pub fn is_reproducible(&self) -> bool {
        let workspace_vars = self.env.get_vars().unwrap_or_default();
        if let Some(value) = workspace_vars.get(SPACES_ENV_IS_WORKSPACE_REPRODUCIBLE) {
